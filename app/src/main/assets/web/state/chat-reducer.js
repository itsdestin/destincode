 ...
 */
function getOrCreateTurn(session) {
    ...
    timeline = [...timeline, { kind: 'assistant-turn', turnId: currentTurnId }];
    return { assistantTurns, timeline, currentTurnId };
}
    ...
    switch (action.type) {
        ...
        case 'SESSION_INIT': {
            if (!next.has(action.sessionId)) {
                next.set(action.sessionId, (0, chat_types_1.createSessionChatState)());
            }
            return next;
        }
        case 'SESSION_REMOVE': {
        ...
        case 'USER_PROMPT': {
            ...
            const isDuplicate = lastFew.some(entry => entry.kind === 'user' && 'message' in entry && entry.message.content === action.content);
            if (isDuplicate) {
        ...
        case 'SHOW_PROMPT': {
            ...
            if (!session) {
                session = (0, chat_types_1.createSessionChatState)();
                next.set(action.sessionId, session);
            }
            const timeline = session.timeline.filter((e) => !(e.kind === 'prompt' && e.prompt.promptId === action.promptId));
            timeline.push({
                kind: 'prompt',
                prompt: {
        ...
        case 'DISMISS_PROMPT': {
            ...
            if (!session)
                return state;
            const timeline = session.timeline.filter((e) => !(e.kind === 'prompt' && e.prompt.promptId === action.promptId && !e.prompt.completed));
            next.set(action.sessionId, { ...session, timeline });
            return next;
        }
        ...
        case 'TRANSCRIPT_USER_MESSAGE': {
            ...
            const isDuplicateT = lastFewT.some(entry => entry.kind === 'user' && 'message' in entry && entry.message.content === action.text);
            if (isDuplicateT) {
                if (!session.isThinking) {
                    next.set(action.sessionId, {
                        ...session, isThinking: true, currentGroupId: null, currentTurnId: null,
        ...
        case 'PERMISSION_REQUEST': {
            ...
            if (!found) {
                ...
                const filteredTimeline = session.timeline.filter((e) => !(e.kind === 'prompt' && !e.prompt.completed));
            ...
            const timeline = session.timeline.filter((e) => !(e.kind === 'prompt' && !e.prompt.completed));
        ...
        case 'HISTORY_LOADED': {
            ...
            const existingTimeline = action.hasMore
                ...
                : session.timeline.filter((e) => {
                    if (e.kind === 'prompt' && e.prompt.promptId === '_history_expand')
                        return false;
                    if (e.kind === 'user' && e.message.id.startsWith('hist-'))
                        return false;
                    if (e.kind === 'assistant-turn' && e.turnId.startsWith('hist-'))
                        return false;
                    return true;
                });
