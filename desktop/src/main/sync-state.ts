...
/**
 * Trigger a force sync by clearing the debounce marker and running sync.sh.
 * The script expects JSON on stdin with a file_path to trigger the correct
 * code path. We use CLAUDE.md as a safe synthetic trigger.
 */
export async function forceSync(): Promise<{
  ...
  // Resolve toolkit root from config to find sync.sh
  const config = (await readJson(configPath)) || {};
  ...
  if (!toolkitRoot) {
    return {
      success: false,
      output: '',
      error: 'Toolkit not installed — toolkit_root not found in config.json',
    };
  }
  ...
  if (!(await fileExists(syncScript))) {
    return {
      success: false,
      output: '',
      error: `sync.sh not found at ${syncScript}`,
    };
  }

  // Run sync.sh with a synthetic trigger file path
  // The script reads JSON from stdin to extract the file_path
  return new Promise((resolve) => {
    ...
    // Pass synthetic stdin JSON — CLAUDE.md triggers the personal data path
    const stdinPayload = JSON.stringify({
      tool_input: {
        file_path: path.join(claudeDir, 'CLAUDE.md'),
      },
    });
