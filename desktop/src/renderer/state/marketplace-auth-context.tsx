...
export function MarketplaceAuthProvider({
  ...
  async function startSignIn() {
    ...
    try {
      ...
      if (!startRes.ok) {
        throw new Error(`sign-in start failed: ${(startRes as any).message ?? "unknown error"}`);
      }
      const { device_code } = startRes.value as { device_code: string; user_code: string; auth_url: string; expires_in: number };

      const deadline = Date.now() + POLL_TIMEOUT_MS;

      // Poll loop — runs until "complete" or timeout
      // Fix: poll() also returns ApiResult<AuthPollResponse> — unwrap the same way
      while (true) {
        ...
        if (!pollRes.ok) {
          throw new Error(`sign-in poll failed: ${(pollRes as any).message ?? "unknown error"}`);
        }
        const pollData = pollRes.value as
          | { status: "pending" }
  ...
  async function signOut() {
    // signOut() returns plain void (not ApiResult-wrapped)
  ...
  return (
    <MarketplaceAuthContext.Provider
      value={{ signedIn, user, signInPending, startSignIn, signOut }}
    >
      {children}
    </MarketplaceAuthContext.Provider>
  );
