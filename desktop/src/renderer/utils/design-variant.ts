...
//
// THIS FILE COMES AND GOES ON PURPOSE. It has now been added and deleted twice
// (organize-UI comparison; icon + popover-placement comparison) and is back for
// a third. That churn IS the lifecycle: it exists only while a design question
// is open, and goes again the moment one is answered. A `dv_` param surviving
...
// a whole surface. Every comparison so far has been a small region inside a
// much larger component, and forking the file N ways to vary that region would
// make drift certain in the lines nobody is comparing — the exact failure the
// spec's "never fork the shipping component" rule exists to prevent.
//
