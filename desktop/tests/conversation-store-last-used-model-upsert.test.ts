// KNOWN-RED repro (2026-08-16, rescued 2026-08-27 from an untracked file in the main checkout).
// WHY it exists: a metadata-only upsert — the shape noteModelUsed sends after a model swap,
// with lastUsedModel but no lastActive — leaves the OLD model on the record. Tracked in
// ROADMAP.md (## Bugs, "metadata-only save doesn't update a session's last-used model").
// This test is expected to FAIL until that bug is fixed; it lives on branch
// test/last-used-model-pin and must not merge red.
