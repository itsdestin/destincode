...
//
// The close prompt after its 2026-07-31 rebuild. Three of these pin decisions
// that are easy to undo by accident:
//
//   1. The tag/note EDITOR is collapsed by default. The dialog's primary button
//      is "Close session"; a search field, a tag list and a textarea in front
//      of it is a form standing between the user and the thing they asked for.
//   2. Reserved flags PRELOAD from the session and round-trip. Priority renders
...
//   3. Complete is still a fresh yes/no on the same result object.
...
describe('CloseSessionPrompt', () => {
  ...
  it('opens collapsed — no tag editor in front of the close button', async () => {
    ...
    mount();
    expect(await screen.findByText('No tags or note')).toBeInTheDocument();
    expect(screen.queryByPlaceholderText('Search or create a tag…')).not.toBeInTheDocument();
    // The empty summary is the same door, worded for having nothing yet.
