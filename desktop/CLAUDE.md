...
- **SkillScanner** (`src/main/skill-scanner.ts`) — Scans installed skills: (1) DestinClaude skills at `~/.claude/plugins/destinclaude/skills/`, (2) marketplace plugins via `~/.claude/installed_plugins.json`. Note: the correct path is `~/.claude/installed_plugins.json` — NOT `~/.claude/plugins/installed_plugins.json` (that was a historical bug, now fixed)
...
- **PluginInstaller** (`src/main/plugin-installer.ts`) — Installs Claude Code plugins to `~/.claude/marketplaces/destincode/plugins/<name>/` and wires them into all four Claude Code registries via `ClaudeCodeRegistry`. Source types: git clone (url), copy from cache (local), sparse checkout (git-subdir)
